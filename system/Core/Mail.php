...
class Mail {
    ...
    public function send() {
        ...
        if ($this->MessageId) {
            $header .= "Message-Id: <30045607.20141017014212.544073f4d35b85.00727728@business.rankedbyreview.com>" . $this->newline;
        }
        ...
        if ($this->protocol == 'spamtest') {

            $res = \Base\Mail\Spamtest::testMail($header . $message, 'long');
            ...
            $sa = new \Spamassassin\Client(
                    array('hostname' => '88.198.77.28')
            );
            try {
                $res = $sa->getSpamReport($header . $message);
                $message_array = explode(PHP_EOL, \Vxd\String::returnBetween('Content analysis details:', '', $res->message));
                array_shift($message_array);
                $message = trim(implode(PHP_EOL, $message_array));
                $res->report = $message;
                return $res;
            } catch (Exception $e) {
                return false;
            }
        } elseif ($this->protocol == 'mail') {
