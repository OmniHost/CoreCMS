...
class ControllerModuleLayerslider extends \Core\Controller {

    public function index($setting) {
        static $module = 0;

        // Add required sources
        $this->document->addStyle('view/plugins/layerslider/css/layerslider.css');
        $this->document->addScript('view/plugins/layerslider/js/layerslider.kreaturamedia.jquery.js');
        $this->document->addScript('view/plugins/layerslider/js/greensock.js');
        $this->document->addScript('view/plugins/layerslider/js/layerslider.transitions.js');
        ...
        if (isset($setting['sections'])) {
            ...
            foreach ($setting['sections'] as $section) {
                ...
                if (isset($section['groups'])) {
                    foreach ($section['groups'] as $group) {
                        ...
                        if (isset($group['image'][$this->config->get('config_language_id')])) {
                            $image = $base_url . 'img/' . $group['image'][$this->config->get('config_language_id')];
                        } else {
                            $image = false;
                        }
                ...
                $data['sections'][$section['sort_order']] = array(
                    ...
                    'thumb_image' => $base_url . 'img/' . $section['thumb_image'],
                    'groups' => $groups
                );
            }
            ...
            $data['module'] = $module++;
             ...
             return $this->render('module/layerslider.phtml', $data);
