...
class ModelToolUpload extends \Core\Model {
	...
	public function deleteUpload($upload_id) {
		$this->db->query("DELETE FROM #__upload WHERE upload_id = '" . (int)$upload_id . "'");
	}

	public function getUpload($upload_id) {
		$query = $this->db->query("SELECT * FROM `#__upload` WHERE upload_id = '" . (int)$upload_id . "'");
	...
	public function getTotalUploads() {
		$sql = "SELECT COUNT(*) AS total FROM #__upload";
