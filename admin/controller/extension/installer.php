...
class ControllerExtensionInstaller extends \Core\Controller {
    ...
    public function index() {
        ...
        $this->data = $data;
        $this->template = 'extension/installer.phtml';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    public function upload() {
        ...
        if (!$json) {
            if (!empty($this->request->files['file']['name'])) {
                if (substr($this->request->files['file']['name'], -8) != '.mod.zip' && substr($this->request->files['file']['name'], -8) != '.mod.xml') {
                ...
                if (file_exists($file)) {
                    ...
                    if ($zip) {
                        ...
                        while ($entry = zip_read($zip)) {
                            ...
                            $file = DIR_ROOT . substr($zip_name, 15);
                            ...
                            $file = DIR_PLUGINS . substr($zip_name, 15);

                            if (is_file($file) && substr($zip_name, 0, 15) == 'upload/plugins/') {
                                $json['overwrite'][] = substr($zip_name, 7);
                            }
                        }
    ...
    public function ftp() {
        ...
        if (!is_dir($directory)) {
            $directory = DIR_UPLOAD . str_replace(array('../', '..\\', '..'), '', $this->request->get['path']) . '/upload/';
        }
