...
class ControllerUserUser extends \Core\Controller {

    private $error = array();

    /**
     * Get list of user
     * @see ControllerUserUser::getList
     */
    ...
    /**
     * Handles teh get list event of users
     */
    protected function getList() {
        ...
        $this->template = 'user/user_list.phtml';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function getForm() {
        ...
        if (isset($this->request->post['profile_img'])) {
            $this->data['profile_img'] = $this->request->post['profile_img'];
        } elseif (!empty($user_info)) {
            $this->data['profile_img'] = $user_info['profile_img'];
        } else {
            $this->data['profile_img'] = 'avatar_generic.jpg';
        }
        
        $this->load->model('tool/image');
       if (isset($this->request->post['profile_img']) && is_file(DIR_IMAGE . $this->request->post['profile_img'])) {
            $this->data['profile_img_preview'] = $this->model_tool_image->resize($this->request->post['profile_img'], 100, 100);
        } elseif ($user_info['profile_img'] && is_file(DIR_IMAGE . $user_info['profile_img'])) {
            $this->data['profile_img_preview'] = $this->model_tool_image->resize($user_info['profile_img'], 100, 100);
        } else {
            $this->data['profile_img_preview'] = $this->model_tool_image->resize('avatar_generic.jpg', 100, 100);
        }
        $this->data['placeholder'] = $this->model_tool_image->resize('avatar_generic.jpg', 100, 100);

        $this->template = 'user/user_form.phtml';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    protected function validateForm() {
        if (!$this->user->hasPermission('modify', 'user/user')) {
            if(!isset($this->request->get['user_id']) || $this->user->getId() != $this->request->get['user_id']){
