...
function PHFileManager(target, thumb, ckeditor, callback, folder) {

    this.thumb = (typeof thumb != 'undefined' && thumb != '' ? thumb : '');
    this.target = (typeof target != 'undefined' && target != '' ? target : '');
    this.ckeditor = (typeof ckeditor != 'undefined' && ckeditor != '' ? ckeditor : '');
    this.callback = (typeof callback != 'undefined' && callback != '' ? callback : '');
    this.searchTerm = '';
    this.directory = [];
    this.page = 1;
    ...
    if (typeof folder != 'undefined' && folder != '') {
        this.directory = folder.split("/");
    }


    this.url = {
        load: 'index.php?p=common/filemanager/load&token=' + this.token,
        move: 'index.php?p=common/filemanager/move&token=' + this.token,
        crop: 'index.php?p=common/filemanager/crop&token=' + this.token,
        remove: 'index.php?p=common/filemanager/delete&token=' + this.token,
        rename: 'index.php?p=common/filemanager/rename&token=' + this.token,
        folder: 'index.php?p=common/filemanager/folder&token=' + this.token,
        localUpload: 'index.php?p=common/filemanager/upload&token=' + this.token,
        remoteUpload: 'index.php?p=common/filemanager/remote&token=' + this.token
    };
...
PHFileManager.prototype.loadDynamicEventHandlers = function () {
    ...
    $('#btnView').click(function () { 
        ...
        window.open(obj.selectedItems[0].href);
    }
    });
    ...
    $("#btnSelect").click(function () {
        ...
        if (obj.selectedItems.length > 0) {
            ...
            if (obj.target != '') {
                $("#" + obj.target).attr('value', obj.selectedItems[0].path).trigger('change');
            }
            if (obj.ckeditor != '') {
                CKEDITOR.tools.callFunction(CKEDITOR.instances[obj.ckeditor]._.filebrowserFn, 'img/' + obj.selectedItems[0].path);
                //$("#" + obj.target).attr('value', obj.selectedItems[0].path).trigger('change');
            }
            if (obj.callback != '') {
               obj.executeFunctionByName(obj.callback, window, obj.selectedItems);
                
               /* if (typeof fn === 'function') {
                    fn(obj.selectedItems[0]);
                }*/
            }
...
PHFileManager.prototype.sendLoad = function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            console.log(data);
            sessionStorage.setItem("lastFolder", data.directory);
        },
        error: function (x) {
            console.log(x.responseText);
    ...
    if (typeof data.images != 'undefined') {
        for (var i = 0; i < data.images.length; i++) {
            ...
            if (data.images[i].type == 'directory') {
                str += '<div class="item-library thumbnail-out draggable droppable" data-info=\'' + JSON.stringify(data.images[i]) + '\'><a href="javascript:void(0)" class="thumbnail directory"><i class="fa fa-folder fa-5x"></i></a><label>' + data.images[i].name.replace(' ', '').substr(0, 20) + '</label></div>';
            } else if (data.images[i].type == 'image') {
                str += '<div class="item-library thumbnail-out draggable" data-info=\'' + JSON.stringify(data.images[i]) + '\'><a href="javascript:void(0)" class="thumbnail image"><img src="' + data.images[i].thumb + '" alt="' + data.images[i].name + '" title="' + data.images[i].name + '" /></a><label>' + data.images[i].name.replace(' ', '').substr(0, 20) + '</label></div>'
            }
            str += '</div>';
        }
    }
...
PHFileManager.prototype.executeFunctionByName = function(functionName, context /*, args */) {
